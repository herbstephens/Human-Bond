...
**After:** The bond's neutral trustee agent, running on 0G Compute, is designed to refuse a proposal unless AgentBook confirms a real, distinct, verified human behind the requesting wallet. This is a live door policy, not a display badge: AgentBook lookups run on World Chain at proposal time, a distinct-humans check confirms the two partner agents belong to two different verified people (not one person puppeting both sides of a "shared" trust), and an unregistered or rogue agent is refused outright — no proposal reaches the Safe. This is the mechanism the trustee agent uses in Scene 4 of the demo: a personal agent asks the trustee to act, the trustee checks the charter and the AgentKit door together, then proposes to the Safe only after both checks pass, and both partners still confirm on their own hito device before funds move.
...
**Qualification:** Changes authorization outright — access to propose against a shared trust is granted or refused based on verified human backing, not a credential check that gates a discount or a badge. Not a registration demo: a registered-but-rogue agent is still refused if the distinct-humans check fails. Working end-to-end on World Chain, demonstrable live.
